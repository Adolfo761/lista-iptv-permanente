...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        url = "https://www.dailymotion.com/player/metadata/video/x80ac48?embedder=https%3A%2F%2Ftelesistema11.com.do%2Fen-vivo%2F"
        headers = {
        ...
        try:
            with urllib.request.urlopen(req, context=ctx, timeout=6) as response:
                ...
                # Fallback a cualquier m3u8 si no se encontró en las calidades estándar
                if not stream_url:
                ...
                if stream_url:
                    ...
                    self.end_headers()
                    self.wfile.write(b"No HLS stream found for Telesistema 11.")
