...
OUT_VOD = "vod_v33.m3u"
OUT_TV = "playlist_v33.m3u"

# --- REGLAS DE RESCATE ---
REGLAS_MIGRACION = [
    # 1. CINE PREMIUM (Detecta CINE - Nombre)
    ...
    # 5. NOTICIAS
    ({"keywords": ["NOTICIAS", "CNN", "BBC", "TELEDIARIO"], "folder": "TV: 🌍 INTERNACIONAL"} )
]

# --- CORRECCIÓN IMPORTANTE: AÑO REAL ---
# Solo acepta años entre 1900 y 2099.
# (1080) YA NO será confundido con un año.
PATRON_AÑO_REAL = re.compile(r'\((?:19|20)\d{2}\)') 
...
def main():
    print("🚀 INICIANDO RESCATE DEFINITIVO V33 (CORRECCIÓN 1080p)...")
    ...
    for entry in items_vod:
        ...
        # 1. FILTRO DE SEGURIDAD: ¿Es una película real con Año (1990-2029)?
        if PATRON_AÑO_REAL.search(name):
            new_vod_list.append(entry) # Se queda en VOD
            continue
            
        # 2. ¿Es un canal para rescatar?
        encontrado = False
        for regla in REGLAS_MIGRACION:
            if any(k in name_upper for k in regla["keywords"]):
                ...
                # Opcional: Limpiar el nombre (quitar prefijos redundantes)
        ...
        if not encontrado:
            # Si no es canal conocido ni película con año, se queda en VOD
            new_vod_list.append(entry)

    # ORDENAR TV
    print("📊 Reorganizando lista de Canales...")
    ...
    print(f"   🚑 Canales Rescatados (Fix 1080p): {moved_count}")
    print(f"   📼 Películas restantes en VOD: {len(new_vod_list)}")
